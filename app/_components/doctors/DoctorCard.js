...
const DoctorCardMain = ({ key, name, specialty, imageSrc }) => (
  <article key={key} className="flex flex-col w-full max-w-[320px] grow pb-2 max-md:mt-5">
    <div className="flex justify-between h-full flex-col pb-5 bg-white shadow-lg md:shadow-2xl rounded-[30px]">
      ...
      <button className="w-[80%] mx-auto py-3 rounded-full text-center text-rose-400 hover:text-rose-800 hover:bg-rose-400 transition-all duration-200 border border-neutral-600 mt-4 text-lg font-semibold">
        Записаться
      </button>
    </div>
  </article>
